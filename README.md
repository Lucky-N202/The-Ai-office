...
- `/advertise` — public pricing page for tool vendors (Verified badge, Featured placement). Pricing is a placeholder in `src/app/advertise/page.tsx` — tune it to whatever the market bears. The `mailto:` CTA is the intended first step (a vendor reaches out, you confirm it's really their tool) — self-serve payment happens next, from `/admin/tools`, once you're ready to charge them. See **Stripe integration** below.

- **Stripe integration** — real, automatic self-serve payment for Featured/Verified, not just a Payment Link you paste in. Deliberately *not* a generic "anyone can pay to feature any tool" link on the public site — that would let someone pay to feature a tool they don't own. The actual flow:
  1. A vendor emails via `/advertise`, you agree on a plan.
  2. In `/admin/tools`, click the shield icon (Verified) or star icon (Featured) next to their specific tool — this calls `/api/checkout` and generates a Stripe Checkout link tied to that exact tool's ID, copied to your clipboard automatically.
  3. Send that link to the vendor. They pay via Stripe's own hosted checkout page.
  4. `/api/webhooks/stripe` receives `checkout.session.completed` and automatically sets `featured` or `verified` on the right tool, revalidating its pages immediately.
  5. **If a Featured subscription is later canceled or fails payment through Stripe's dunning process**, the `customer.subscription.deleted` webhook automatically un-features that tool — so a lapsed payment doesn't mean a permanently-featured listing.

  Setup: create a Stripe account, add two Products — a $99/mo recurring Price for Featured and a $49 one-time Price for Verified (or whatever you land on) — and copy their Price IDs into `STRIPE_FEATURED_PRICE_ID`/`STRIPE_VERIFIED_PRICE_ID`. Create a webhook endpoint in the Stripe dashboard pointing at `https://yourdomain.com/api/webhooks/stripe`, listening for `checkout.session.completed` and `customer.subscription.deleted`, and copy its signing secret into `STRIPE_WEBHOOK_SECRET`. Without `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET` set, `/api/checkout` returns a clear "not configured" error rather than crashing — safe to deploy before you've set this up.
